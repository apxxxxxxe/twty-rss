...
func showTweets(tweets []Tweet, asjson bool, rssTitle string, verbose bool) {
	if asjson {
		...
		feed := &feeds.Feed{
			Title:       rssTitle,
			Link:        &feeds.Link{Href: "https://twitter.com"},
			Description: "generated by twty-rss",
		...
		for _, tweet := range tweets {
			...
			feed.Items = append(feed.Items, &feeds.Item{
				Title:       fmt.Sprint(tweet.User.Name, " / @", tweet.User.ScreenName, " ", text),
...
func main() {
	...
	flag.BoolVar(&asrss, "rss", false, "show tweets as rss")
...
`)
	...
	if asrss {
		rssTitle = "Your Timeline"
		titleContender := []string{list, user}
		for _, tc := range titleContender {
			if tc != "" {
				rssTitle = tc
				break
			}
	...
	if len(search) > 0 {
		...
		for {
			...
			if len(res.Statuses) > 0 {
				showTweets(res.Statuses, asjson, rssTitle, verbose)
		...
		showTweets(tweets, asjson, rssTitle, verbose)
	} else if list != "" {
